...
## Ithacus-Specific Recovery

- **SQLite store corruption:** Rebuild from `store.ts` schema initialization
- **Node:sqlite errors:** Verify Node.js version (>= 22.13 required)
- **TypeScript compilation:** Run `npm run build` for type-checking, use `node --experimental-strip-types` for runtime
- **Import resolution:** Ensure ESM imports have `.js` extensions
- **Zero-network violations:** Check for `fetch`, `http`, `https`, `net`, `dns` imports

## References

- `docs/AGENT_GUARDRAILS.md` — Core safety protocols
- `.guardrails/prevention-rules/pattern-rules.json` — Pattern rules
- `.claude/hooks/pre-commit.sh` — Pre-commit validation
- `scripts/regression_check.py` — Regression detection
